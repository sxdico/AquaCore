...
public class BansMenu extends SwitchableMenu {
    ...
    public List<Slot> getSwitchableSlots(Player player) {
        ...
        List<Punishment> punishments = punishData.getPunishments().stream().sorted(Comparator.comparingLong(Punishment::getAddedAt).reversed())
                .filter(punishment -> punishment.getPunishmentType() == PunishmentType.BAN).collect(Collectors.toList());
