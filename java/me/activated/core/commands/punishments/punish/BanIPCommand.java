...
public class BanIPCommand extends BaseCommand {
    ...
    @Command(name = "banip", inGameOnly = false, permission = "punishments.command.ban", aliases = {"tempbanip", "ipban", "iptempban"})
    public void onCommand(CommandArgs command) {
        ...
        Tasks.runAsync(plugin, () -> {
            if (args.length < 2) {
                sender.sendMessage(Color.translate("&cUsage: /ipban <player> [optional:<duration>] <reason> [-s]"));
                return;
            }
            ...
            if (targetData.getPunishData().isBanned()) {
                sender.sendMessage(PunishmentsLanguage.BAN_ALREADY_BANNED.toString().replace("<user>", target.getName()));
